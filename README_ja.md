...
<h3 align="center">ComfyUI 用モジュラー UI プラットフォーム</h3>
...
100% AI によってコーディングされた、ComfyUI 上で動作する UI プラットフォームです。<br>
**ガジェット**という独立した UI モジュール単位で簡単に機能を追加できるように設計されています。

画面下部のタブバーに各ガジェットが収納され、ワークフローを離れることなく、パラメータ操作・画像管理・モデル管理・パラメータスイープといった作業が 1 つのパネルから完結します。
...
- `drawer-sample-anima-advanced` — SAM3、SeedVR2を含むより実践的なワークフロー（追加のモデル、カスタムノードが必要です）
...
- バイパス軸 — ノードの ON/OFF 自体をスイープの軸として使用可能
...
- 初回起動時に SQLite ベースの検索インデックスを自動生成するため、ワークフローのメタデータ（ノードタイプ、ノードタイトル、入力値等）での全文検索が可能
...
- **CivitAI 同期** — SHA256 ハッシュによるモデル照合でメタデータ＆プレビュー画像を取得可能
- **ノードマッチング** — **インフォカード**から、ワークフロー内の対応するローダーノードに適用可能（サブグラフや Combo Clone も含む全ノードをスキャン）
...
設定パネルから辞書の作成・編集・ON/OFF が管理できます。これらの処理は専用ノードを追加する方式ではなく、ComfyUI のキュー投入時にワークフロー全体のテキスト入力へ自動的に適用されます。つまり、通常のプロンプトノード、DrawerControls 経由のテキスト、その他の文字列ウィジェットに対して、ノード無しでコメント除去とワイルドカード展開が行われます。
...
| **DrawerControls1 / 4 / 8 / 12** | コンパクトなパラメータハブ。接続済みの出力だけが Deck 上の `int` / `float` / `combo` / `bool` / `string` コントロールとして表示されます。単行は `string \| ラベル`、複数行は `string \| ラベル \| multiline` のみを使い、combo 候補は接続先 widget から取得されます |
