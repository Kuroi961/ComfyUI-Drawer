...
100% AI によってコーディングされた、モバイルでも扱いやすい ComfyUI 用 UI プラットフォームです。

**ガジェット**という独立した UI モジュール単位で簡単に機能を追加できるように設計されています。

画面下部のタブバーに各ガジェットが収納され、デスクトップでもモバイルでも、ワークフローを離れることなく、パラメータ操作・画像管理・モデル管理・パラメータスイープといった作業が 1 つのパネルから完結します。
