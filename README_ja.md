...
- **Zip モード** — 隣接する2軸を「ペア」として同期ステップさせる。直積にならずにペアごとに1枚ずつ生成（例：X+Y を Zip にして `(モデル, LoRA)` のペアをまとめてスイープ）
...
| **Zip モード** | 各軸ペア間に「Zip」チェックボックスを表示。有効にすると両軸を同期ステップで反復。X+Y を3値ずつ Zip にすると9枚→3枚。値数の不一致は no-op でパディングされ、プリフライト時に警告 |
...
- カスタムノードが Drawer のインデックス contributor を登録していれば、サードパーティのカスタムメタデータも検索可能。独自フィールドは `namespace[value]` または `namespace:key[value]` 形式で検索できます（例: `myPlugin:tags[black hair]`）
- 検索対象、日付範囲、ファイルサイズのフィルターに対応し、モバイル向けの2段検索ツールバーで操作可能
- Gallery の検索欄でも、ノードタイプ、ユーザー辞書、Danbooru 辞書、登録済みカスタムメタデータキーのオートコンプリートを利用可能
...
- Python 拡張から Gallery のメタデータ provider、検索インデックス contributor、メタデータパネル contributor、辞書 provider を登録できます。既存のカスタムメタデータを Drawer 専用の保存形式に寄せず、検索・表示・補完へ接続できます

完全な API リファレンスは [GADGET_API.md](GADGET_API.md) を参照してください。

| ドキュメント | 説明 |
|-------------|------|
| [docs/USER_GUIDE_ja.md](docs/USER_GUIDE_ja.md) | README より詳しい日本語ユーザーガイド |
