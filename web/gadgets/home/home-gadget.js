...
export class HomeGadget extends GadgetBase {
    ...
    async #renderSystemInfo() {
        ...
        let sysInfo = {};
        try {
            const resp = await this.bridge.fetchApi('/system_stats');
    ...
    async #renderChangelog() {
        ...
        try {
            const res = await this.bridge.fetchApi('/drawer/changelog', {
                cache: 'no-store',
                signal: this.#changelogAbort.signal,
            });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const entries = this.#parseChangelog(await res.text()).slice(0, CHANGELOG_ENTRY_LIMIT);
