...
<p align="center">
  ...
  <a href="#为什么需要-drawer">为什么 Drawer?</a> •
  <a href="#安装">安装</a> •
  <a href="#示例工作流">示例</a> •
  <a href="#内置小工具">小工具</a> •
...
Drawer 内部采用模块化结构，可以把工作流控制、媒体管理、模型浏览、绘图工具以及自定义扩展集中在同一个位置。

---

## 为什么需要 Drawer？

在桌面端，ComfyUI-Drawer 会为 ComfyUI 画布增加一层类似遥控器的操作界面：保留打开的节点图，同时把实际需要触碰的参数、素材和检查工具集中到底部 Drawer 中操作。

在移动端或小屏幕上，这个 Drawer 本身可以成为主要的工作界面。它把参数控制、输出查看、模型浏览、遮罩编辑、提示词词典和 XYZ 检验放到触手可及的位置，让创作中的操作变得实际可用。

不同于 APP mode，Drawer 的设计目标不是把工作流变成独立应用，而是在编辑、测试和反复调整工作流时提供可持续使用的 UI。
...
- 文件名搜索无需索引即可使用；prompt、workflow、节点类型和节点标题等元数据搜索使用显式创建的 SQLite 搜索索引
...
- **无需节点** — 通配符展开和注释移除会在入队时自动应用

可在设置面板中创建、编辑、启用或禁用词典。无需在工作流中添加通配符节点、词典节点或预处理节点。普通 prompt 节点、通过 DrawerControls 传入的文本，以及其他字符串 widget，都会在 ComfyUI 入队时自动使用注释移除和通配符展开。
...
ComfyUI-Drawer 采用分离授权：

- 实现代码：[GPL-3.0-or-later](LICENSE)
- 公共规范文档（`ARCHITECTURE.md`, `CONVENTIONS.md`, `GADGET_API.md`）：CC0-1.0
- 小工具模板（`docs/gadget-template.js`）：MIT
