...
扫描期间，普通队列提交会被暂时阻止，避免外部操作混入扫描。Drawer 也会抑制会改变当前工作流状态的操作，包括 Deck 控制、把媒体发送到 LoadImage/LoadImageMask、从上下文菜单打开 Mask Editor，以及从 Model Viewer 信息卡应用模型。
如果检测到服务器断开连接，扫描会中止。
...
如果 output/input 中有大量文件，创建索引可能需要一些时间。Drawer 会显示创建进度，并在用户手动开始的创建或同步完成时显示 toast。创建或同步失败时，会显示包含原因的错误对话框。最近一次快速估算的结果会尽量复用，因此开始创建索引时不容易立刻重复同一轮计数。
...
视频缩略图以及视频/音频元数据会在可用时使用外部 `ffmpeg` / `ffprobe` 程序。如果 `ffmpeg` 不可用，视频缩略图会显示占位图。
